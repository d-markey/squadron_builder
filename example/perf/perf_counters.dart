...
class PerfCounter {
  PerfCounter(this.duration, this.skippedTicks, this.maxSkippedTicks,
      this.totalTicks, this.maxDeviation);
  ...
  PerfCounter operator -(PerfCounter origin) => PerfCounter(
        ...
        maxDeviation >= origin.maxDeviation
            ? maxDeviation
            : origin.maxDeviation,
      );

  PerfCounter operator /(int n) => PerfCounter(duration ~/ n, skippedTicks ~/ n,
      maxSkippedTicks, totalTicks ~/ n, maxDeviation);
}
...
class PerfCounters {
  ...
  Duration get maxDeviation => Duration(
      microseconds: math.max(
          ...
          math.max(echo.maxDeviation.inMicroseconds,
              perf.maxDeviation.inMicroseconds)));

  int get maxSkippedTicks => math.max(fib.maxSkippedTicks,
